.. zephyr:code-sample:: memory_chunks
   :name: Memory Chunks
   :relevant-api: rtio

   Implement chunk wise processing of a larger memory area using RTIO.

Overview
********

This sample application demonstrates the use of the :ref:`rtio` framework for
processing a larger memory area in smaller chunks.
...
.. zephyr-app-commands::
   :zephyr-app: samples/subsys/rtio/memory_chunks
   :board: native_sim
   :goals: build run
   :compact:

When running, the output on the console shows the detailed steps
of processing the memory area.
